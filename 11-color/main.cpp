...
int main(/*int argc, char **argv*/) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // draw cube
    cube_shader.use();
    cube_shader.set_uniform("objectColor", 1.0f, 0.5f, 0.31f);
    cube_shader.set_uniform("lightColor", 1.0f, 1.0f, 1.0f);
    update_camera_view(cube_shader);
    update_projection(cube_shader);
    glBindBuffer(GL_ARRAY_BUFFER, VAOs[0]); // VAOs[1] is cube VAO
    set_cube_model_and_draw(cube_shader);
    // draw lighting
    lighting_shader.use();
    update_camera_view(lighting_shader);
    update_projection(lighting_shader);
    glBindBuffer(GL_ARRAY_BUFFER, VAOs[1]); // VAOs[1] is lighting VAO
    set_light_model_and_draw(lighting_shader);
